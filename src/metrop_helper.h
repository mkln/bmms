...
inline double log_mvn_density(arma::vec x, arma::vec mean, arma::mat covar){
...
inline double modular_loglik1(arma::vec& y, arma::vec& marglik_mean, arma::mat& varloglik, arma::vec& sigmasq_scales, int n_stages){
  double result = log_mvn_density(y, marglik_mean, varloglik) - arma::accu(log(sigmasq_scales.subvec(0, n_stages-1)));
  return result;
}

inline double modular_loglik2(arma::vec& y, arma::mat& mean_post, arma::mat& inv_var_post, double a, double b){
...
inline double modular_loglik0(arma::vec& y, double a, double b){
  int n = y.n_elem;
...
inline double modular_loglikn(const arma::vec& x, const arma::mat& Si){
  int p = Si.n_cols;
...
inline Module::Module(arma::mat& X_full, arma::vec& e, arma::mat& X, double g_prior,
...
inline void Module::redo(const arma::vec& e,
                  const arma::mat& Jpre, 
                  const arma::mat& Jnow,
...
inline ModularLinReg::ModularLinReg(const arma::vec& yin, 
                             const arma::mat& Xin, 
                             double g, 
...
inline void ModularLinReg::add_new_module(arma::vec& new_splits){
...
inline void ModularLinReg::delete_last_module(){
  n_stages -= 1;
...
inline void ModularLinReg::redo(){
...
inline void ModularLinReg::change_all(arma::field<arma::vec>& new_splitseq){
...
inline void ModularLinReg::change_module(int whichone, arma::vec& new_splits){
...
inline double totsplit_prior2_ratio(int tot_split_prop, int tot_split_orig, int norp, int ss, double lambda_prop){
...
inline double split_struct_ratio2(const arma::field<arma::vec>& proposed, const arma::field<arma::vec>& original,
...
inline double totsplit_prior_ratio(int tot_split_prop, int tot_split_orig, int norp, int ss, double lambda_prop){
...
inline double splitpar_prior(double x, int tot_split, int norp, int ss){
  //lambda_prop is 1/variance;
  double means = pow(2, ss);
...
inline double totstage_prior_ratio(int tot_stage_prop, int tot_stage_orig, int norp, int curr_n_splits, int direction){
  return 1.0;
}
